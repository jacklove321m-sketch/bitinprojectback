...
//            $price = bc_mul($amount,$price,8);
    ...
    public static function userBuyCoint($userId,$buyCurrencyId,$payCurrencyId,$amount,$price){
       ...
       // $costPrice = bc_mul($price,$amount);
         ...
         $costPrice = $amount;
        if($qut->now_price) $num = $amount / $qut->now_price;
        DB::beginTransaction();
        try{
//            $price = bc_mul($amount,$price,8);
            ...
            //生成
            CoinTrade::newTrade($userId,CoinTrade::TRADE_TYPE_BUY,$buyCurrencyId,$payCurrencyId,$num,$qut->now_price,$price);//$amount
            DB::commit();
        }catch (\Exception $e){
            DB::rollBack();
            throw $e;
    ...
    // if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',json_encode($tradeList));
        foreach($tradeList as $trade){
            ...
            try{
                ...
                $wallet = UsersWallet::where("user_id", $trade->u_id)
                  ...
                  //  if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',json_encode($wallet).PHP_EOL,FILE_APPEND);
                ...
                $targetWallet = UsersWallet::where("user_id", $trade->u_id)
                    ...
                    //  if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',time()."--".json_encode($targetWallet).PHP_EOL,FILE_APPEND);
                if(!$wallet || !$targetWallet){
                    throw new \Exception(sprintf('订单%s找不到用户钱包',$trade->id));
                }
                $costPrice = bc_mul($trade->target_price,$trade->trade_amount,8);
                
                // if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',$costPrice.PHP_EOL,FILE_APPEND);
              ...
              //   if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',$amount.PHP_EOL,FILE_APPEND);
                change_wallet_balance($targetWallet,
                    ...
                    '币币交易成功');
                $trade->status = 2;
 // if($currencyId==49)file_put_contents('/www/wwwroot/crypto/public/tt1.txt',$trade->status.PHP_EOL,FILE_APPEND);
