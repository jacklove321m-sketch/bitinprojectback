...
class MicroTradeLogic
{
    ...
    public static function addOrder($param)
    {
      ...
      //  file_put_contents('/www/wwwroot/crypto/public/m.txt',json_encode($param).PHP_EOL,FILE_APPEND);
        try {
            DB::beginTransaction();
            //检测用户是否存在
            ...
            */
            ...
            $order_data = [
                ...
                //'complete_at' => date('Y-m-d H:i:s',time()+$seconds->seconds),
    ...
    public static function close($match_id)
    {
        try {
           ...
           */
           ...
           // file_put_contents('/www/wwwroot/crypto/public/t1.txt',$match_id."---".json_encode($closing_oreders).PHP_EOL,FILE_APPEND);
            ...
            foreach ($closing_oreders as $key => $value) {
                ...
                if ($value->profit_type == 1) {
                    //结算本金和利息
                    $profit_ratio = bc_div($value->profit_ratio, 100);  //旧的赢注释
                    $capital = $value->number;//旧的赢注释
                    $fact_profit = bc_mul($capital, $profit_ratio); //旧的赢注释
                    
                    //$fact_profit = $value->number;  赢 全部赢
                   ...
                   /*
                   $capital = $value->number;;
                    // $fact_profit = -$value->number;  //原本的 
                    $fact_profit = $value->number * ($value->profit_ratio / 100);  //新的
                    $fact_profit = -$fact_profit;  //新的
                  ...
                  //  file_put_contents('/www/wwwroot/crypto/public/t1.txt',$value->profit_type."---".$loss_ratio."---".$money_new."--".$fact_profit);
                    $memo =  '秒合约订单,亏损结算';
                }
