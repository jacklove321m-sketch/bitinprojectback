...
//        var_dump($lever_transaction->toArray());die;
         ...
         public function dataget()
    {
        ...
        $currency_id = Input::get("currency_id");
        $user_wallet = UsersWallet::where('currency', $legal_id)->where('user_id', $user_id)->first();
        $data['lever_balance']=$user_wallet ? $user_wallet->lever_balance : 0;
        $Marginratio=DB::table('lever_transaction')->where('legal', $legal_id)->where('user_id', $user_id)->where('status',1)->sum('Marginratio');
        $data['fact_profits']=DB::table('lever_transaction')->where('legal', $legal_id)->where('user_id', $user_id)->where('status',1)->sum('fact_profits');
        $data['marginratio']=$Marginratio == 0 ? 0 : ($data['lever_balance']+$Marginratio)/$Marginratio;
        return $this->success($data);
    }
...
//    }
    ...
    public function submit()
    {
        ...
        $now = time();
        $user_lever = 0;
      //  file_put_contents('/www/wwwroot/amex.theice.vip/public/t.txt',date('Y-m-d H:i:s')."--------status:".$status."----------target_price:".$target_price.PHP_EOL,FILE_APPEND);
...
//        //挂单委托(限价交易)价格取用户设置的
        ...
        //交易手数转换
        $lever_share_num = $currency_match->lever_share_num ?? 1;
        $num = $all_money = bc_mul($share, $lever_share_num); 
        ...
        //初始保证金=手数*1000
        $all_money=bc_mul($share,1000);
        //手续费=手数*30
        $trade_fee=bc_mul($share,30);
        DB::beginTransaction();
        try {
            $legal = UsersWallet::where("user_id", $user_id)
                ->where("currency", $legal_id)
            ...
            $caution_money = $all_money;//bc_div($all_money, $multiple); //保证金
            
            if($type==1){   //强平价格
              $QiangpingPrice  =$last_price-$caution_money;
            }
            if($type==2){
            ...
            $Marginratio =bc_div(bc_add($user_lever,$caution_money),$caution_money);  //保证金率
            
         //   $QiangpingPrice = $type==1?  //需要计算
            ...
            $lever_transaction =LeverTransaction::where('user_id',$user_id)
                ...
                ->where('status',1)
                ->first();
            if($lever_transaction && $status!=0){
                 $update['share']=bcadd($lever_transaction->share,$share);
                $update['sheng_share']=bcadd($lever_transaction->sheng_share,$share);
                $update['origin_caution_money']=bcadd($lever_transaction->origin_caution_money,$caution_money);
                $update['caution_money']=bcadd($lever_transaction->caution_money,$caution_money);
                $update['trade_fee']=bcadd($lever_transaction->trade_fee,$trade_fee);
                $update['Marginratio']=bc_div(bc_add($user_lever,$caution_money),$caution_money);
                // var_dump($update);die;
                 $result= LeverTransaction::where('user_id',$user_id)
                ->where('type',$type)
                ->where('id',$lever_transaction->id)
                ->where('currency',$currency_id)
                ->where('legal',$legal_id)->where('status',1)->update($update);
                
                
                $logdata['user_id']=$lever_transaction->user_id;
                $logdata['type']=$lever_transaction->type;
                $logdata['origin_price']=$lever_transaction->origin_price;
                $logdata['price']=$lever_transaction->price;
                $logdata['update_price']=$lever_transaction->update_price;
                $logdata['share']=bcadd($lever_transaction->sheng_share,$share);
                $logdata['level_transaction_id']=$lever_transaction->id;
                $logdata['ping_share']=$share;
                $logdata['number']=$lever_transaction->number;
                $logdata['origin_caution_money']=bcmul($share,1000);
                $logdata['number']=$lever_transaction->caution_money;
                $logdata['caution_money']=bcadd($lever_transaction->caution_money,$caution_money);;
                $logdata['QiangpingPrice']=$lever_transaction->QiangpingPrice;
                $logdata['currency']=$lever_transaction->currency;
                $logdata['legal']=$lever_transaction->legal;
                $logdata['multiple']=$lever_transaction->multiple;
                $logdata['trade_fee']=bcmul($share,30);
                $logdata['transaction_time']=time();
                $logdata['create_time']=$lever_transaction->create_time;
                $logdata['status']=$lever_transaction->status;
                $logdata['orderbuy_type']=0;
            DB::table('lever_transaction_log')->insert($logdata);
           ...
           // $lever_transaction->overnight = $overnight;
            $lever_transaction->origin_price = $origin_price;
            $lever_transaction->price = $origin_price;//$fact_price;
            $lever_transaction->update_price = $last_price;
            $lever_transaction->share = $share;
            $lever_transaction->sheng_share=$share;
            $lever_transaction->number = $num;
            $lever_transaction->origin_caution_money = $caution_money;
            $lever_transaction->caution_money = $caution_money;
            $lever_transaction->Marginratio = $Marginratio;
            $lever_transaction->QiangpingPrice = $QiangpingPrice>0?$QiangpingPrice:0;
            $lever_transaction->currency = $currency_id;
            $lever_transaction->legal = $legal_id; 
            $lever_transaction->multiple = $multiple;
            $lever_transaction->trade_fee = $trade_fee;
            ...
            $logdata['price']=$lever_transaction->price;
            $logdata['update_price']=$lever_transaction->update_price;
            $logdata['share']=$lever_transaction->share;
            $logdata['level_transaction_id']=$lever_transaction->id;
            $logdata['ping_share']=0;
            $logdata['number']=$lever_transaction->number;
            $logdata['origin_caution_money']=$lever_transaction->origin_caution_money;
            $logdata['number']=$lever_transaction->caution_money;
            $logdata['caution_money']=$lever_transaction->Marginratio;
            $logdata['QiangpingPrice']=$lever_transaction->QiangpingPrice;
            $logdata['currency']=$lever_transaction->currency;
            $logdata['legal']=$lever_transaction->legal;
            $logdata['multiple']=$lever_transaction->multiple;
            $logdata['trade_fee']=$lever_transaction->trade_fee;
            $logdata['transaction_time']=$lever_transaction->transaction_time;
            $logdata['create_time']=$lever_transaction->create_time;
            $logdata['status']=$lever_transaction->status;
            $logdata['orderbuy_type']=0;
            ...
            $result = change_wallet_balance(
                ...
                -$caution_money,
                AccountLog::LEVER_TRANSACTION,
                '提交' . $currency_match->symbol . '杠杆交易,价格' . $last_price . ',扣除保证金',
                false,
                0,
                0,
                serialize([
...
//            var_dump($PP);die;
    ...
    public function close()
    {
        $user_id = Users::getUserId();
        $id = Input::get("id");
        $ping_share = Input::get("ping_share");
        if (empty($id)) {
            return $this->error("参数错误");
        }
        DB::beginTransaction();
        try {
            ...
            if ($lever_transaction->sheng_share < $ping_share) {
                throw new \Exception("平仓数量不能大于持有数量");
            }
            ...
            $return = LeverTransaction::leverClose($lever_transaction,$ping_share);
            if (!$return) {
