...
class MarketHour extends Model
{
    ...
    public static function setEsearchMarket($market_data)
    {
        ...
        $currencyInfo=DB::table('currency')->where('name',$market_data['base-currency'])->first();
        // if($currencyInfo){
        //     if($currencyInfo->enabled==1){
        //         $sisValue=0;
        //         $sisValue =  $currencyInfo->min_unit * $currencyInfo-> count;
                      
        //         $sisValue  =round($sisValue,4);
        //         if($sisValue>0){
        //                     $market_data['high']    = round($market_data['high'] + $sisValue,4);
        //                     $market_data['close']   = round($market_data['close'] + $sisValue,4);
        //                     $market_data['low']     = $market_data['low'] + $sisValue;
                           
        //                     // $market_data['change'] = ($market_data['close']  - $market_data['open'])/$market_data['open']*100;
                           
        //                     // $market_data['change'] = round($market_data['change'],4); 
        //         }
        //     }
        // }
    ...
    public static function getAndSetEsearchMarket($market_data)
    {
        ...
        if (isset($result['_source'])) {
          ...
          //  bc_comp($data['high'], $price) < 0 && $market_data['high'] = $price; //更新最高价
           ...
           if($market_data['base-currency'] =='BTC')
        var_dump('get and set '.$market_data['close'].'===='.$data['close']); 
        }
...
//        dd($res);
        ...
        // var_dump('get and set '.$market_data['close'].'===='.$data['close']);
            ...
            if($currency_model->enabled==1 && date('s',time())==00){
                // $sis =  bcmul($currency_model->yujicontact,$currency_model->min_unit/100,4);
                 $das=DB::table('currency_contact')->where('currency_id',$currency_model->id)->where('time',date('YmdHi',time()))->first();
                 
                
                if($currency_model->yujicontact !=$currency_model->min_unit && !$das){
                    $sis=bcmul($item['open'],$currency_model->count/$currency_model->min_unit/100,4);
                    $yujicontact=$currency_model->yujicontact + 1;
                }else{
                    $sis=0;
                    $yujicontact=$currency_model->yujicontact;
                }
                
                
                 
                 $oncontact=bcadd($currency_model->oncontact,$sis,4);
                 
                 
                //  if($currency_model->yujicontact <= 0 && $oncontact <=$currency_model->yujicontact){
                //      $oncontact=$currency_model->yujicontact;
                //  }
                ...
                //  }
                 ...
                 Currency::where(['name' => $base_currency])->update(['oncontact'=>$oncontact,'yujicontact'=>$yujicontact]);
    ...
    public static function batchEsearchMarket($base_currency, $quote_currency, $price, $time)
    {
        ...
        foreach ($periods as $key => $period) {
            ...
            if (isset($result['_source'])) {
                ...
                // self::setEsearchMarket($data);//20240215
            } else {
                continue;//2024015
                //dd($result);
            ...
            //     $sisValue=0;
            //     $sisValue =  $currency->min_unit * $currency-> count;
                      
            //     $sisValue  =round($sisValue,4);
            //     if($sisValue>0){
                    
            //                 $data['high']    = round($data['high'] + $sisValue,4);
            //                 $data['close']   = round($data['close'] + $sisValue,4);
            //                 $data['low']     = $data['low'] + $sisValue;
                            
                            
            //                 $kline_data['high']    = round($kline_data['high'] + $sisValue,4);
            //                 $kline_data['close']   = round($kline_data['close'] + $sisValue,4);
            //                 $kline_data['low']     = $kline_data['low'] + $sisValue;
