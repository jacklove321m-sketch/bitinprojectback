...
class WsConnection
{
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        $rand = rand(1,4);
        if($period=='1week'&&$rand==2||$period=='1mon'&&$rand==2){
            //推送币种的日行情(带涨副)
    ...
    protected function onMarketKline111($con, $data, $match)
    {
        $topic = $data->ch;
        $msg = date('Y-m-d H:i:s') . ' 进程' . $this->worker_id . '接收' . $topic . '行情' . PHP_EOL;
        list($name, $symbol, $detail_name, $period) = explode('.', $topic);
        ...
        $currencyInfo=DB::table('currency')->where('name',$currency_match->currency_name)->first();
           $sisValue  =$currencyInfo->oncontact;
                if($sisValue  != 0){
                            // $tick->high    = round($tick->high + $sisValue,8);
                            // $tick->close   = round($tick->close + $sisValue,8);
                            // $tick->low     = round($tick->low + $sisValue,8);
    ...
    protected function onMarketDepth($con, $data, $match)
    {
        ...
        if($rand==1||$rand==2){//期货 或外汇
            ...
            'legal_id' => $currency_match->legal_id,
            'legal_name' => $currency_match->legal_name,
            'bids' => array_slice($data->tick->bids, 0, 20), //买入盘口
            'asks' => array_slice($data->tick->asks, 0, 20), //卖出盘口
         ];
        }
    ...
    //盘口数据回调
    protected function onMarketDepth111($con, $data, $match)
    {
        $topic = $data->ch;
        $subscribed_data = $this->getSubscribed($topic);
        $currency_match = $subscribed_data['match'];
        ...
        if ($currency_match->currency_name == 'BTC' && $currency_match->legal_name == 'USDT') {
            $tick_bids = array_slice($data->tick->bids, 0, 50);
            $tick_asks = array_slice($data->tick->asks, 0, 50);
            $self_match_ids = self::getSelfMatch();
            $redis = RedisService::getInstance(4);
            foreach ($self_match_ids as $id) {
                $currencyMatch = CurrencyMatch::query()->find($id);
        ...
        if ($currency_match->market_from == 2) {
            ...
            $depth_data = [
                'type' => 'market_depth',
                'symbol' => $currency_name . '/' . $currency_match->legal_name,
                ...
                'legal_id' => $currency_match->legal_id,
                'legal_name' => $currency_match->legal_name,
                'bids' => array_slice($data->tick->bids, 0, 50), //买入盘口
                'asks' => array_slice($data->tick->asks, 0, 50), //卖出盘口
            ];
            $symbol_key = $currency_match->currency_name . '.' . $currency_match->legal_name;
            self::$marketDepthData[$symbol_key] = $depth_data;
        }
    ...
    public function sendDepthData()
    {
        ...
        foreach ($market_depth as $depth_data) {
            $currencyInfo=DB::table('currency')->where('name',$depth_data['base-currency'])->first();
            if($currencyInfo){
                $sisValue  =$currencyInfo->oncontact;
                if($sisValue>0){
    ...
    public function sendMatchTradeData()
    {
        ...
        foreach ($market_trade as $trade_data) {
            // $trade_data['data'][0]->price=111111;
...
//                echo '期权时间:' . time() . ', Symbol:' . $key . '.' . $period . '数据' . PHP_EOL;
                if ($period == '1min') {
                    ...
                    // if($c_m->open_microtrade == 1){
                     ...
                     HandleMicroTrade::dispatch($symbol['forward_data'])->onQueue('micro_trade:handle');// 期货 外汇
