...
class WsConnection
{
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        foreach ($sys_matches as $k=>$v){
           ...
           $to = time();
            ...
            if(!empty($t)) {
           // file_put_contents('/www/wwwroot/crypto/public/t1.txt',json_encode($t));
            $f1 = "config/kline/".$t->itime."_kline_1min_".strtolower($v["name"]).".php";
            $f2 = "config/kline/".$t->itime."_daymarket_".strtolower($v["name"]).".php";
           
            
            if(!file_exists('/www/wwwroot/crypto/config/'.$f1)){
                
                $f1 = "config/kline_1min_".strtolower($v["name"]).".php";
            }
            
            if(!file_exists('/www/wwwroot/crypto/config/'.$f2)){
           ...
           // UserChat::sendChat($adt["libra_data"]);
            ...
            /*
             $params = [
                ...
                'currency_id' => $kline_data_new["currency_id"],
                'currency_name' => $kline_data_new["currency_name"],
                'now_price' =>$kline_data_new["close"],
                'now' =>  microtime(true)
            ];
            //价格大于0才进行任务推送
            if (bc_comp($kline_data_new["close"], 0) > 0) {
    ...
    protected function onMarketDepth($con, $data, $match)
    {
        ...
        if ($currency_match->currency_name == 'BTC' && $currency_match->legal_name == 'USDT') {
            $tick_bids = array_slice($data->tick->bids, 0, 20);
            $tick_asks = array_slice($data->tick->asks, 0, 20);
            $self_match_ids = self::getSelfMatch();
            $redis = RedisService::getInstance(4);
            foreach ($self_match_ids as $id) {
                $currencyMatch = CurrencyMatch::query()->find($id);
    ...
    public function onMessage($con, $data)
    {
       ...
       // 	file_put_contents("onMessage.txt", json_encode($data) . PHP_EOL, FILE_APPEND);
