...
*/
    ...
    protected static function handleUserLever($user_id, $legal_id, $currency_id = 0)
    {
       ...
       if (empty($user_id)) {
            ...
            try {
            DB::beginTransaction();
            $last_price = self::getLastPrice($legal_id, $currency_id);
            $lever_transactions=self::where('user_id',$user_id)->where('currency',$currency_id)->where('legal',$legal_id)->where('status',self::TRANSACTION)->get();
           
            foreach ($lever_transactions as $lever_transaction) {
            ...
            if($lever_transaction->fact_profits + $lever_transaction->caution_money >0){
                //收益+保证金不不小于0 跳过
                continue;
            }
            $sheng=$lever_transaction->share - $ping_share;
            
            //更新状态
            $lever_transaction->update_price = $last_price;
            $lever_transaction->update_time = microtime(true);
            if($lever_transaction->sheng_share - $ping_share ==0){
            ...
            $lever_transaction->caution_money=($sheng)*1000;
            
            $lever_transaction->complete_time = microtime(true);
            $lever_transaction->sheng_share=$sheng;  
             $result = $lever_transaction->save();
                ...
                $logdata['handle_time']=time();
                $logdata['complete_time']=time();
                $logdata['status']=3;
                $logdata['orderbuy_type']=1;
...
//            }
    ...
    public static function leverClose($lever_transaction,$ping_share, $is_follow = false)
    {
        ...
        try {
            ...
            $sheng=$lever_transaction->sheng_share - $ping_share;
             
            // var_dump($sheng.'----'.$lever_transaction->sheng_share .'----'.$ping_share);die;
            //更新状态
            $lever_transaction->update_price = $last_price;
            $lever_transaction->update_time = microtime(true);
            if($sheng==0){
            ...
            //计算盈亏
            $profit = $lever_transaction->profits;
            $change = bc_add($ping_share*1000, $profit); //算上本金
            //从钱包处理资金
            $pre_result = bc_add($legal_wallet->lever_balance, $change);
            $diff = 0;
            //是否余额不够扣除
            ...
            if($lever_transaction->type==1){   //强平价格
              $QiangpingPrice  =$last_price-$lever_transaction->caution_money;
            }
            if($lever_transaction->type==2){
                
              $QiangpingPrice  =$last_price+$lever_transaction->caution_money;
            }
            $Marginratio=$lever_transaction->caution_money !=0 ? bc_div(bc_add($legal_wallet->lever_balance,$lever_transaction->caution_money),$lever_transaction->caution_money):0;  //保证金率
