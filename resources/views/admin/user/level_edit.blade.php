...
@section('page-content')
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">会员卡图</label>
            <div class="layui-input-block">
                <button class="layui-btn" type="button" id="img_cover_btn1">选择图片</button>
                <br>
                <img src="{{$level->img ?? ''}}" id="img_cover1" width="100" class="cover" style="display: @if(!empty($level->img)){{"block"}}@else{{"none"}}@endif;max-width: 200px;height: auto;margin-top: 5px;">
                <input type="hidden" name="img" id="cover1" value="{{$level->img ?? ''}}">
            </div>
        </div>
    ...
    layui.use(['element', 'form', 'layer','upload'], function () {
        ...
        var uploadInst1 = upload.render({
         ...
         var uploadInst2 = upload.render({
            elem: '#img_cover_btn1' //绑定元素
            ,url: '{{URL("api/upload")}}?scene=admin' //上传接口
            ,done: function(res) {
                console.log(res);
                //上传完毕回调
                if (res.type == "ok"){
                    $("#cover1").val(res.message)
                    $("#img_cover1").show()
                    $("#img_cover1").attr("src",res.message)
                } else{
                    alert(res.message)
                }
            }
