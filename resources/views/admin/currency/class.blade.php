...
@section('scripts')
    ...
    <script type="text/html" id="is_display">
        <input type="checkbox" name="is_display" value="@{{d.id}}" lay-skin="switch" lay-text="是|否" lay-filter="is_display"  @{{ d.is_display == 1 ? 'checked' : '' }}>
    </script>
    <script>
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,toolbar: '#toolbar'
                ,url: '{{url('admin/currency_listclass')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'id', width:60, sort: true}
                    ,{field: 'sort', title: '排序', width:90}
                    ,{field: 'pcname', title: '名称', width:90}
                    ,{field:'is_display', title:'显示', width:85, templet: '#is_display', unresize: true}
        ...
        //监听是否购买保险操作
            ...
            //是否开启币币划转
            form.on('switch(is_display)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'{{url('admin/class_transfer')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.open({
                            title: '添加分类'
                            ,type: 2
                            ,content: '/admin/currency_addclass'
                            ,area: ['480px', '650px']
                        });
                        break;
                    default:
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'{{url('admin/currency_delclass')}}',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
