...
<div class="layui-form">
    <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加','{{url('admin/lending/add')}}')">添加</button>
...
<script id="barOpe" type="text/html">
    ...
    <script type="text/html" id="statustml">
        @{{d.status==1 ? '<span class="layui-badge layui-bg-green">'+'启用'+'</span>' : '' }}
        @{{d.status==0 ? '<span class="layui-badge layui-bg-red">'+'未启用'+'</span>' : '' }}
    ...
    <script>
        layui.use(['table','form'], function(){
            ...
            var demo = table.render({
                elem: '#demo'
                ,toolbar: '#toolbar'
                ,url: '{{url('admin/lending_list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'id', width: 80, sort: true}
                  // ,{field: 'title', title: '放款机构', minWidth:60}
                    ,{field: 'days', title: '期限', width: 80}
                   // ,{field: 'amount', title: '起始金额', width: 80}
                  //  ,{field: 'amax', title: '最大金额', width: 80}
                    ,{field: 'bilv', title: '利息（%）', width: 150}
                    ,{field: 'percent', title: '验资百分比', width: 150}
                    ,{field: 'opaymentlv', title: '滞纳金百分比（%）', width: 150}
                    ,{field: 'fwamount', title: '总服务费', width: 120}
                     
 
                    ,{field: 'hk_type', title: '还款方式', width: 150} 
            ...
            table.on('tool(test)', function (obj) {
                ...
                if (layEvent === 'edit') {
                    ...
                    //     type: 2
                    //     ,title: '编辑'
                    //     ,content: '/admin/lending/editDualView' + '?id=' + data.id
                    //     ,area: ['960px', '650px']
                    // });
                    var index = layer.open({
                        title: '编辑'
                        ,type: 2
                        ,content: '{{url('/admin/lending/editLendingView')}}?id=' + data.id
                        ,area: ['960px', '500px']
                    });
                }
