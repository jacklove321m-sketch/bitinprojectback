...
<style>
    ...
    <div class="layui-inline">
        ...
        <input type="checkbox" name="review_status" value="@{{d.id}}" lay-skin="switch" lay-text="是|否" lay-filter="sexDemo" @{{ d.review_status == 2 ? 'checked' : '' }} > <span >@{{ d.review_status == 4 ? '审核失败' : '' }}</span>
        </script>
                ...
                window.onload = function() {
                    ...
                    layui.use(['element', 'form', 'layer', 'table'], function () {
                        ...
                        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                            ...
                            }else if (layEvent === 'error') { //失败
                                layer.confirm('真的要不通过吗？', function (index) {
                                    //向服务端发送删除指令
                                    $.ajax({
                                        url: "{{url('admin/user/real_error')}}",
                                        type: 'post',
                                        dataType: 'json',
                                        data: {id: data.id},
                                        success: function (res) {
